...
# ── 读取 OpenFAST 输出文件 ──
outfile = '5MW_OC3Trpd_DLL_WSt_WavesReg_Case2.out'
...
def compute_acceleration(dx, dt=0.04):
    """Compute acceleration from displacement: low-pass filter + 2nd derivative."""
...
ax.set_title('工况二：三结点X方向位移对比（规则波 Hs=8m, Tp=10s）')
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('工况二_结点位移对比.png', dpi=150)
print('Saved: 工况二_结点位移对比.png')
...
for name, ch in nodes.items():
    ...
    ax1.set_title(f'工况二：{name} {ch["z"]} — X方向位移与加速度')
...
ax1.set_title('工况二：规则波面时程 (100-200s)')
ax1.grid(True, alpha=0.3)
...
ax2.set_title('工况二：规则波频谱')
ax2.legend()
ax2.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('工况二_波面时程频谱.png', dpi=150)
print('Saved: 工况二_波面时程频谱.png')
